...
if False:
    print("This might take ages to run, just bear with me...")
...
if False:
    print("Run softmax on MNIST mod 3 test error: ",run_softmax_on_MNIST_mod3())
...
def run_softmax_on_MNIST_pca(train_x,train_y,test_x,test_y,temp_parameter=1):
    theta, cost_function_history = softmax_regression(train_x, train_y, temp_parameter, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
    plot_cost_function_over_time(cost_function_history)
    test_error = compute_test_error(test_x, test_y, theta, temp_parameter)
    # Save the model parameters theta obtained from calling softmax_regression to disk.
    ...
    return test_error
if False:
    print("Softmax with pca test error: ",run_softmax_on_MNIST_pca(train_pca,train_y,test_pca,test_y))
...
if True:
    print("Softmax with pca 10 and cubic features test error: ",run_softmax_on_MNIST_pca(train_cube,train_y,test_cube,test_y))